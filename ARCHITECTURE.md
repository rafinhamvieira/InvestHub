...
- **Permissão do autor é verificada na rota; regra sobre o alvo, na política.** `MANAGE_ROLES` guarda a troca de cargo — `MANAGE_USERS` sozinha não basta, porque mexer em cargo é a única ação capaz de fabricar um administrador. `admin-policy` continua respondendo "quem pode agir sobre quem". A matriz em `/admin/roles` é gerada do mapa, nunca escrita à mão: o que a tela mostra é o que a aplicação verifica.
- **Configuração em banco guarda só a diferença.** `platform_settings` tem linha apenas para o que foi alterado; sem linha, vale o `.env` ou o padrão de código, e apagar a linha desfaz a mudança. É o que impede o problema clássico de configuração em banco: a tabela que, uma vez populada, esconde de onde o valor realmente vem. O registro em `config/platform-settings.ts` é a fonte única de rótulo, unidade e **limites** — campo livre num painel de configuração é pé de cabra, e cada faixa tem o porquê escrito. Segredos e URLs de conexão continuam só no `.env`; parâmetros de outros containers não aparecem na tela, porque a aplicação não os lê.
- **A série histórica pega carona no healthcheck.** O Docker chama `/api/health` a cada 15 s; a amostra é gravada na cadência dela mesma (5 min por padrão), com o intervalo guardado em memória — reinício do processo produz amostra imediata, e é bom que produza. Nenhum agendador novo. A coleta nunca lança e tem prazo curto: falha ao medir não pode fazer o orquestrador julgar o container doente. **Buraco na série é registro de queda** — com o banco fora do ar não há como gravar a amostra que diria isso —, e por isso a disponibilidade é contada contra o número esperado de amostras, não contra as coletadas.
- **Log em arquivo, não no banco.** `stdout` sempre, mais uma linha JSON no volume `./logs` — assim o registro sobrevive à recriação do container e não depende do Postgres, que é justamente o que pode ter caído. A gravação é enfileirada, nunca lança e nunca chama o próprio `logger` (seria recursão); falha desliga o sink e avisa uma vez no console. A leitura varre do fim do arquivo para trás com teto de bytes, e declara truncamento em vez de fingir que leu tudo. `VIEW_APPLICATION_LOGS` é porta mais estreita que a da trilha: o contexto de um log é livre, e não dá para prometer o que **não** vai aparecer nele.
...
**A trilha não tem chave estrangeira para `users`.** `ON DELETE SET NULL` fazia o banco atualizar `audit_logs` ao excluir uma conta, e o trigger de imutabilidade recusava — nenhuma conta com histórico saía da base. Permitir o UPDATE quebraria a cadeia, porque `userId` e `actorId` entram no payload assinado. As colunas guardam o id sem constraint; o e-mail denormalizado mantém a linha legível; nome e e-mail atuais são resolvidos por consulta no repositório. Chave estrangeira era elegância referencial paga com mutabilidade numa tabela que se define por não ser mutável.
...
Painel administrativo profissional, em 10 etapas, uma por vez com confirmação: **1. Auditoria (entregue)** · **2. Dashboard administrativo (entregue)** · **3. Gestão de usuários (entregue)** · **4. Backup com ensaio de restauração (entregue)** · **5. Logs da aplicação (entregue)** · **6. Telas de RBAC (entregue)** · **7. Monitoramento (entregue)** · **8. Configurações da plataforma (entregue)** · 9. Central de segurança (inclui checkpoints de auditoria em armazenamento externo WORM) · 10. Auditoria administrativa.
